 ...
 */
export async function sendDonationReceipt({ donorName, donorEmail, amount, transactionId }) {
  // Si le template de reçu n'est pas configuré, on échoue silencieusement
  if (!TEMPLATE_ID || !SERVICE_ID || !PUBLIC_KEY) {
    console.warn('[DonationReceipt] EmailJS non configuré pour les reçus — ignoré.')
    return
  }
  ...
  const templateParams = {
    ...
    organization_email: 'clubenvironnementaldeguinee@gmail.com',
    to_email:           donorEmail,
    reply_to:           'clubenvironnementaldeguinee@gmail.com',
  }
