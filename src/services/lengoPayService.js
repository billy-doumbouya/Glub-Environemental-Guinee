...
const API_KEY     = import.meta.env.VITE_LENGOPAY_API_KEY
const MERCHANT_ID = import.meta.env.VITE_LENGOPAY_MERCHANT_ID
const BASE_URL    = import.meta.env.VITE_LENGOPAY_BASE_URL || 'https://api.lengopay.com'
 ...
 */
export async function initiatePayment({ amount, phone, donorName, donorEmail, transactionId }) {
  if (!API_KEY || !MERCHANT_ID) {
    throw new Error('Clés LengoPay manquantes. Vérifier VITE_LENGOPAY_API_KEY et VITE_LENGOPAY_MERCHANT_ID dans .env')
  }
  ...
  if (!response.ok) {
    throw new Error(data?.message || data?.error || `Erreur LengoPay: ${response.status}`)
  }
...
export async function checkPaymentStatus(transactionId) {
  const response = await fetch(`${BASE_URL}/v1/payments/status/${transactionId}`, {
    headers: {
      'Authorization': `Bearer ${API_KEY}`,
      'X-Merchant-ID': MERCHANT_ID,
    },
